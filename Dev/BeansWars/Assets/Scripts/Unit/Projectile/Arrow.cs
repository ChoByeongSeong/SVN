...
public class Arrow : Projectile
{
    ...
    IEnumerator Collusion()
    {
        ...
        while (alive)
        {
            ...
            {
                // 유닛 마스크
                Collider2D col = Physics2D.OverlapCircle(head.position, colRad, LayerMask.GetMask("Unit"));
    ...
    IEnumerator Destroy()
    {
        ...
        while (true)
        {
            ...
            {
                ProjectilesPool.instance.DestroyArrow(this);
                break;
            }
        }
    }
    ...
    private void OnDrawGizmos()
    {
        if (head != null)
        {
            Gizmos.DrawWireSphere(head.position, colRad);
        }
    }
}