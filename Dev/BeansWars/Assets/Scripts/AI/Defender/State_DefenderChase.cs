...
public class State_DefenderChase : State
{
    StateController_Defender sc;
    ...
    public State_DefenderChase(StateController controller) : base(controller)
    {
        sc = (controller as StateController_Defender);
    }
    ...
    public override IEnumerator Update()
    {
        ...
        if (IsInAttackrange())
        {
            walkAnimName = "heavywalk";

            // 스피드 증가량
            sc.speedPercent = sc.speedReductionAmount;

            // 원본 데이터를 찾아서.
            // mass 를 설정한다.
            string defenderName = YBEnum.eUnitName.Defender.ToString();
            float mass = DataManager.GetInstance().GetUnitData(defenderName).mass;

            sc.unit.rb2d.mass = mass * sc.massIncreaseAmount;

            sc.unit.status.armor = sc.armorAmount;
        }
    ...
    void OnPathFound(Vector3[] waypoints, bool pathsuccessful)
    {
        ...
        if (pathsuccessful)
        {
            ...
            path = PathRequestManager.instance.pathPool.GetPath(waypoints, sc.Position, turnDst, stoppingDst);

            if (follow != null)
            {
                sc.StopCoroutine(follow);
                follow = null;
            }
    ...
    IEnumerator FollowTarget()
    {
        ...
        controller.modelAnim.Play(walkAnimName);
